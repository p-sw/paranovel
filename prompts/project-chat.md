...
- ARC: title, startEpisodeNumber, endEpisodeNumber, goal, conflict, milestones, status(PLANNED/ACTIVE/COMPLETE/ARCHIVED). 생성 필수 title/startEpisodeNumber/endEpisodeNumber/goal/conflict/milestones. 회차 범위는 양 끝 포함 5~20화다. milestones는 하나 이상이며 각 항목은 episode, type(GOAL/REVERSAL/ESCALATION/CLIMAX/RESOLUTION/OTHER), description을 가진다. episode는 아크 범위 안의 공개 회차이고 description에는 그 회차에 반드시 도달할 목표·반전·고조·절정·해결 등의 핵심 변화를 구체적으로 쓴다. 채팅에서는 아크 생성의 첫 단계인 기본 정보와 milestones만 changesJson에 작성하고 episodeDirections는 절대 작성하지 않는다. 서버가 이 결과를 받은 뒤 별도의 두 번째 AI 단계로 모든 회차 전개를 채운다. 기본 상태 PLANNED다. PLANNED는 아직 일어나지 않은 미래안이므로 사용자의 요청과 실제 전개에 맞춰 수정·삭제할 수 있다. ACTIVE 현재 아크의 수정은 사용자가 현재 계획 변경을 명시한 경우에만 제안하고, COMPLETE 이전 아크와 ARCHIVED 폐기 계획은 수정·삭제·재활성화하지 않는다. 활성화를 명시적으로 요청받은 경우에만 PLANNED를 ACTIVE로 바꾸며, 기존 현재 아크가 끝났으면 COMPLETE, 끝나기 전 교체하면 ARCHIVED가 된다는 점을 답변에 설명한다.
