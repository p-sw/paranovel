...
- 회차 에디터 상단의 ‘편집 AI’에서 집필 모델과 대화하며 장면을 쓰거나 선택한 부분을 수정합니다. 선택문이 없으면 커서 위치(선택한 적이 없으면 원고 끝)에 삽입문을 제안합니다. 수정안은 검토 후 적용하며, 대화는 회차별로 저장되고 프로젝트 AI 채팅과 분리됩니다. 적용 전 원고가 바뀌면 현재 원고를 기준으로 다시 요청해야 합니다.
