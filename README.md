...
- 회차 화면에서 ‘하이라이트 삽화 생성’을 누르면 AI가 장면 하나를 골라 AnimeAPI로 이미지 한 장을 만들고, ‘읽기 미리보기’에서 해당 문단 뒤에 표시합니다. 삽화의 위치 변경·재생성·삭제도 지원합니다.
...
| `ANIMEAPI_API_KEY` | 없음 | 회차 하이라이트 이미지 생성 키. 장면 선택에는 기존 OpenRouter 키도 필요 |
| `IMAGE_STORAGE_PATH` | SQLite 파일 옆 `images/` | 생성 이미지의 영구 저장 디렉터리. Docker 기본 경로는 `/data/images` |
...
서버에 `ANIMEAPI_API_KEY`와 `OPENROUTER_API_KEY`를 설정하면 회차 화면의 삽화 생성 버튼을 사용할 수 있습니다. 생성 전 대기 중인 본문 저장을 완료하고, 집필 모델이 저장된 회차와 확정 정사를 읽어 문단·구도·프롬프트를 선택합니다. 서버는 [AnimeAPI 생성 API](https://animeapi.ai/docs/api-reference)에 `model: "ultra-max"`, `enhance: true`를 항상 보냅니다. `prompt`는 최대 1,000자이고 `orientation`과 `allowNSFW`는 AI가 선택합니다. AnimeAPI 문서상 `enhance`와 `allowNSFW`는 `anime` 모델에만 적용되며 `ultra-max`는 자연어 프롬프트와 SFW 출력을 사용합니다.

회차당 현재 삽화는 한 장입니다. 본문과 삽화는 별도로 저장되어 글자 수·회차 기억·텍스트 백업에 이미지 문법이 섞이지 않습니다. 생성 도중에도 원고를 편집할 수 있고, 다른 회차로 이동하거나 새로고침해도 저장된 생성 상태를 확인할 수 있습니다. 생성 후 본문이 바뀌면 검토 필요 표시가 나타납니다. 기존 문단이 유일하게 남아 있으면 삽화도 그 위치를 따라가며, 삭제되거나 중복되어 위치를 판별할 수 없으면 ‘위치 변경’으로 문단을 다시 선택합니다. 위치를 바꾸어도 삽화를 만든 당시의 본문 기록은 유지됩니다.

재생성은 새 파일 저장까지 성공해야 기존 삽화를 교체합니다. 동일 요청의 재전송은 저장된 상태로 처리하며, AnimeAPI의 응답이 불명확하거나 서버가 재시작된 경우 유료 생성 요청을 자동 재시도하지 않습니다. 이미지 URL을 받은 뒤 다운로드나 저장만 실패했다면 ‘이미지 저장 다시 시도’로 기존 결과를 다시 저장합니다. 이미지 다운로드에는 공개 HTTPS 주소, 래스터 형식, 20MiB 크기 및 30초 시간 제한을 적용합니다.

이미지는 SQLite 옆 `images/` 디렉터리(Docker에서는 `/data/images`)에 저장합니다. 삽화·회차·프로젝트 삭제 시 해당 파일도 정리합니다. 백업할 때는 SQLite와 이미지 디렉터리를 함께 보관하세요. `IMAGE_STORAGE_PATH`로 별도 디렉터리를 지정하는 경우 해당 경로도 영구 볼륨으로 보존해야 합니다.
...
Compose는 React 정적 자산과 NestJS API가 포함된 단일 컨테이너를 실행하고, named volume `paranovel_data`에 `/data/paranovel.sqlite`와 `/data/images`를 보존합니다. 컨테이너를 삭제해도 volume을 삭제하지 않는 한 소설 데이터와 삽화는 남습니다. 운영 데이터는 volume 전체를 정기적으로 백업하세요.
...
- API 키를 프런트엔드 코드나 브라우저 저장소에 넣지 마세요. OpenRouter·Tavily·AnimeAPI 호출은 NestJS 서버에서만 수행합니다.
